...
private static ResourceBundle rb;
	...
	public static void display(int dbIncomeId){
		...
		ResultSet rs = outcome.selectIncomeById(dbIncomeId);
		
		//resource
		rb = ResourceBundle.getBundle("lang", Locale.getDefault());
		Stage window = new Stage();
		window.setTitle(rb.getString("outcomeEdit"));
		...
		DatePicker dp = customDatePicker.make(330, "dd/MM/YYYY", "textfield", rb.getString("dateFormat"), DBdate);	
		...
		try {
			fID = rs.getInt(3);
			fNAME = rs.getString(4) + " " + rs.getString(5);
		} catch (SQLException e2) {	}
		...
		ComboBox<Product> productsCombo = productsC.selectByIndex2(330, new Product(pID, pNAME));
		...
		a.setOnAction(e ->{
			...
			if(newTime==0 || monInt==0 || fmid==0 || curId==0 || proId==0 || addText==null){
				...
				int resultX = database.outcome.editOutcomeToDB(dbIncomeId, newTime, monInt, fmid, curId, proId, addText);
				if(resultX==1){ 
		...
		URL url = editOutcome.class.getResource("/image/favicon.png");
